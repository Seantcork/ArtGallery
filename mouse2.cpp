...
//vector of segments for drawing.
...
/* ****************************** */
/*  polygon stored in global variable poly  */
void initialize_polygon() {
...
//takes the points of the polygon and creates them in segments
//this makes sure that my code works
...
//draws visible polygon via triangulation.
void draw_visible(){
  if(poly_init_mode ==1){
    return;
  }
...
void keypress(unsigned char key, int x, int y) {
  ...
  case 'q':
    ...
    //expected behaviour: press 'r', then click on the points you
